...
class Product {
	...
	public function frontend_scripts() {
		...
		if ( '1' == $this->gallery_options['image_zoom'] ) {
		...
		$warrows           = ( '1' == $this->gallery_options['slider_nav'] ) ? 'true' : 'false';
		$w_thumb_arrows    = ( '1' == $this->gallery_options['thumb_nav'] ) ? 'true' : 'false';
		$wzoom             = $this->gallery_options['image_zoom'];
		$wslider_animation = $this->gallery_options['slider_animation'];
		$slider_dots       = ( 1 == $this->gallery_options['dots'] ) ? 'true' : 'false';
		$wautoPlay         = ( '1' == $this->gallery_options['slider_autoplay'] ) ? 'true' : 'false';
		$autoplay_timeout  = $this->gallery_options['autoplay_timeout'];

		$slider_autoplay_pause = ( '1' == $this->gallery_options['slider_autoplay_pause'] ) ? 'true' : 'false';
		$wslider_thubms        = $this->gallery_options['thumb_to_show'];
		$thumb_scroll_by       = $this->gallery_options['thumb_scroll_by'];
		$slider_adaptiveHeight = ( '1' == $this->gallery_options['slider_adaptiveHeight'] ) ? 'true' : 'false';
		$wslider_mouse_draging = ( '1' == $this->gallery_options['slider_dragging'] ) ? 'true' : 'false';
		$wslider_infinity_loop = ( '1' == $this->gallery_options['slider_infinity'] ) ? 'true' : 'false';

		$slider_rtl = ( is_rtl() ) ? 'true' : 'false';
		$nextArrow  = ( 'false' == $slider_rtl ) ? "nextArrow:'<i class=\"flaticon-right-arrow\"></i>'," : '';
		$prevArrow  = ( 'false' == $slider_rtl ) ? " prevArrow:'<i class=\"flaticon-back\"></i>'," : '';
		...
		if ( '1' == $this->gallery_options['lightbox_picker'] ) {
